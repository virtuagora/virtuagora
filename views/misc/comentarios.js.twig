...
function agregarSinComentarios(){
	$("#row-ver-mas").remove();
	$("#comentarios").append(
		row()
		.append(
			columnasQuote(4)
			.addClass("large-centered text-center")
			.append(
				$("<h5>")
				.html("~ Fin de los comentarios ~")
			)
			.append(
				$("<div>")
				.attr("id","div-comentar-aux")
				.addClass("text-center")
				.append(
					$("<div>")
					.attr("id","bt-comentar-aux")
					.addClass("button tiny radius expand")
					.append(
						$("<i>")
						.addClass("fa fa-comments fa-fw fa-lg")
					)
					.append(
						"¿Desearias dejar un comentario?"
					)
				)
			)
		)
...
function crearArrayComentarios(){
	var comentarios = new Array();
	var par = new Array();
	var respuestas = new Array();
	// un Comentarios = [ [Comentario, [ [RTA1],[RTA2] ...] ] , [Comentario, [ [RTA1],[RTA2] ...] ], [Comentario, [ [RTA1],[RTA2] ...] ], ]
	var number = 0 + Math.floor(Math.random() * 30);
	for(var i=0; i<number; i++){
		...
		for(var j=0; j<number2; j++){
			var number3 = 120 + Math.floor(Math.random() * 300);
			respuestas.push(
				crearRespuesta(i,number3,"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum elementum metus eget accumsan placerat. Donec urna arcu, ornare fringilla vulputate tristique, pharetra a augue.","#","http://lorempixel.com/32/32/sports/","Respuesta ID"+number3,"Sabado 30 de Junio 2005",20,15)
			)
		}
		par.push(respuestas)
		comentarios.push(par);
	}
	return comentarios;
}
