...
$app->get('/', function () use ($app) {
    if ($app->session->exists()) {
        ...
        $app->render('usuario/portal.twig', array('contenidos' => $contenidos->toArray(),
                                                  'esModerador' => $app->session->hasRole('mod'),
                                                  'esFuncionario' => $app->session->hasRole('fnc')));
    } else {
        $app->render('registro/registro.twig');
    }
});
...
$app->get('/propuesta/:id', function ($id) use ($app) {
    ...
    $app->render('contenido/propuesta/ver.twig', array('propuesta' => array_merge($contenido->toArray(),
                                                                                  $propuesta->toArray())));
});

$app->post('/propuesta/:id/votar', function ($idPropuesta) use ($app) {
    if (!$app->session->hasRole('usr')) {
        throw new BearableException('No tiene permiso para realizar esta acción', 403);
    }
    $validator = new Augusthur\Validation\Validator();
    $validator
        ->add_rule('postura', new Augusthur\Validation\Rule\InArray(array(-1, 0, 1)))
        ->add_rule('idPropuesta', new Augusthur\Validation\Rule\NumNatural());
    $req = $app->request;
    $data = array_merge(array('idPropuesta' => $idPropuesta), $req->post());
    if (!$validator->is_valid($data)) {
        throw (new TurnbackException())->setErrors($validator->get_errors());
    }
    ...
    $propuesta = Propuesta::findOrFail($idPropuesta);
